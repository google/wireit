...
// Write the self-signed cert lazily on first setup() call.
let certWritten: Promise<void> | undefined;
function ensureCertWritten() {
  certWritten ??= (async () => {
...
async function setup() {
  ...
  const server = new FakeGitHubActionsCacheServer(authToken, {
  ...
  return {
    ...
    [Symbol.asyncDispose]: async () => {
      await Promise.all([server.close(), rig[Symbol.asyncDispose]()]);
    },
  };
}
...
test('recovers from reservation race condition', async () => {
  ...
  assert.deepStrictEqual(cmdA.numInvocations, n);
  assert.deepStrictEqual(server.metrics, {
    getCacheEntry: n,
    createCacheEntry: n,
    putBlobBlock: 1,
    putBlobBlockList: 1,
