...
export const registerCommonCacheTests = (
  ...
  test('caches single file', async () => {
    ...
    // Initial run with input v0.
    {
      ...
      assert.deepStrictEqual(await rig.read('output'), 'v0');
    }
    ...
    // Input changed to v1. Run again.
    {
      ...
      assert.deepStrictEqual(await rig.read('output'), 'v1');
    }
  ...
  test('caches large file', {timeout: 30_000}, async () => {
    ...
    // Change input back to v0. The large file should be restored from cache.
    {
      ...
      assert.deepStrictEqual((await exec.exit).code, 0);
      assert.deepStrictEqual(cmdA.numInvocations, 2);
      assert.deepStrictEqual(await rig.read('output'), fileContent);
    }
  });
  ...
  test('caching follows glob patterns', async () => {
    ...
    // Initial run with input v0.
    {
      ...
      assert.deepStrictEqual(await rig.read('output/excluded/foo'), 'excluded');
    ...
    // Input changed back to v0. Output should be cached.
    {
      ...
      assert.deepStrictEqual(await rig.read('output/0/a'), 'v0');
      assert.deepStrictEqual(await rig.read('output/0/b'), 'v0');
      assert.deepStrictEqual(await rig.read('output/0/c/d/e'), 'v0');
      assert.deepStrictEqual(await rig.read('output/excluded/foo'), 'excluded');
    }
    ...
    // Input changed back to v1. Output should be cached.
    {
      ...
      assert.ok(!(await rig.exists('output/0/b')));
      assert.ok(!(await rig.exists('output/0/c/d/e')));
      assert.deepStrictEqual(await rig.read('output/1/a'), 'v1');
      assert.deepStrictEqual(await rig.read('output/1/b'), 'v1');
      assert.deepStrictEqual(await rig.read('output/1/c/d/e'), 'v1');
      assert.deepStrictEqual(await rig.read('output/excluded/foo'), 'excluded');
    }
  });
  ...
  test('caching supports glob re-inclusion', async () => {
    ...
    // Initial run with input v0.
    {
      ...
      // have been cleaned.
      assert.deepStrictEqual(await rig.read('output/subdir/excluded'), 'v0');
    ...
    // Input changed back to v0. Output should be cached.
    {
      ...
      assert.deepStrictEqual(await rig.read('output/subdir/excluded'), 'v0');
      assert.deepStrictEqual(await rig.read('output/subdir/reincluded'), 'v0');
    }
    ...
    // Input changed back to v1. Output should be cached.
    {
      ...
      assert.deepStrictEqual(await rig.read('output/subdir/excluded'), 'v0');
      assert.deepStrictEqual(await rig.read('output/subdir/reincluded'), 'v1');
    }
  });
  ...
  test('cleans output when restoring from cache even when clean setting is false', async () => {
    ...
    // Initial run with input v0.
    {
      ...
      assert.deepStrictEqual(res.code, 0);
      assert.deepStrictEqual(cmdA.numInvocations, 1);
      assert.deepStrictEqual(await rig.read('output/both'), 'v0');
      assert.deepStrictEqual(await rig.read('output/only-v0'), 'v0');
    ...
    // Input changed to v1. Run again.
    {
      ...
      assert.deepStrictEqual(res.code, 0);
      assert.deepStrictEqual(cmdA.numInvocations, 2);
      assert.deepStrictEqual(await rig.read('output/both'), 'v1');
      assert.ok(!(await rig.exists('output/only-v0')));
      assert.deepStrictEqual(await rig.read('output/only-v1'), 'v1');
    }
  ...
  test('caches symlinks to files without following them', async () => {
    ...
    assert.deepStrictEqual(await rig.read('symlink'), 'foo');
    ...
    // Change input back to v0. Restored from cache.
    {
      ...
      assert.deepStrictEqual(await rig.readlink('symlink'), 'target');
      ...
      assert.deepStrictEqual(await rig.read('symlink'), 'bar');
    }
  });
  ...
  test('caches symlinks to directories without following them', async () => {
    ...
    // Initial run with input v0.
    {
      ...
      assert.deepStrictEqual(await rig.read('target'), 'v0');
...
❌ package.json:9:17 Output files must be within the package: ${JSON.stringify(
    ...
    assert.deepStrictEqual(cmdA.numInvocations, 0);
