...
import {flakyTest} from './util/rig-test.js';
import {WireitTestRig} from './util/test-rig.js';
import {IS_WINDOWS} from '../util/windows.js';

test('simple consumer and service with stdout', async () => {
...
test('service with standard and service deps', async () => {
  await using rig = await WireitTestRig.setup();
  //  consumer
  //     |
  //     v
...
test('standard scripts are killed when service exits unexpectedly', async () => {
  await using rig = await WireitTestRig.setup();
  // consumer
  //    |
  //    v
...
test('service remembers unexpected exit failure for next start call', async () => {
  await using rig = await WireitTestRig.setup();
  //     entrypoint
  //     /        \
  //    v          v
...
test('service shuts down when service dependency exits unexpectedly', async () => {
  await using rig = await WireitTestRig.setup();
  // consumer
  //    |
  //    v
...
test('persistent service and dependency starts and runs until SIGINT', async () => {
  // service2 //    v //    | // service1
  await using rig = await WireitTestRig.setup();
  // This test uses standard logger output to ensure that
  // certain operations happen in the right order.
...
test('service fingerprint is trackable despite never having outputs', async () => {
  await using rig = await WireitTestRig.setup();
  // consumer
  //    |
  //    v
...
test('service waits for log before being considered started', async () => {
  // service //    v //    | // standard
  await using rig = await WireitTestRig.setup();
  const standard = await rig.newCommand();
  const service = await rig.newCommand();
...
test(`can abort a service while it's waiting on a dependency`, async () => {
  // standard //    v //    | // service
  await using rig = await WireitTestRig.setup();
  const service = await rig.newCommand();
  const standard = await rig.newCommand();
  ...
  const standardInv = await standard.nextInvocation();
  await exec.waitForLog(/0% \[0 \/ 2\] \[1 running\] \[0 services\] standard/);
  exec.kill();
  // we close properly
  await standardInv.closed;
  await exec.exit;
