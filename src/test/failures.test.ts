...
test('runs one script that fails', async () => {
...
test('runs one non-root script that fails', async () => {
  await using rig = await WireitTestRig.setup();
  rig.env['WIREIT_LOGGER'] = 'quiet';
  const cmdA = await rig.newCommand();
  await rig.write({
...
test('dependency chain in one package that fails in the middle', async () => {
  await using rig = await WireitTestRig.setup();
  // a --> b* --> c
  const cmdA = await rig.newCommand();
...
test('dependency chain in one package that fails in nested dependency', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  const cmdB = await rig.newCommand();
  const cmdC = await rig.newCommand();
...
for (const envSetting of ['no-new', undefined]) {
  test(`don't start new script after unrelated failure when WIREIT_FAILURES=${
    envSetting ?? '<unset>'
  }`, async () => {
    await using rig = await WireitTestRig.setup();
    //   main
...
test("don't start new script after unrelated failure with constrained parallelism in no-new mode", async () => {
  await using rig = await WireitTestRig.setup();
...
test('allow unrelated scripts to start after failure in continue mode', async () => {
  await using rig = await WireitTestRig.setup();
  //   main
  //    /\
  //   |  |
...
test('kill running script after failure in kill mode', async () => {
  await using rig = await WireitTestRig.setup();
  //   main
  //    / \
  //   |   |
