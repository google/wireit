...
test('invoked through npx', async () => {
  await using rig = await WireitTestRig.setup();
...
test('negative parallelism', async () => {
  await using rig = await WireitTestRig.setup();
  await rig.write({
    'package.json': {
      scripts: {
...
test('nonsense parallelism', async () => {
  await using rig = await WireitTestRig.setup();
  await rig.write({
    'package.json': {
      scripts: {
...
test('nonsense WIREIT_CACHE', async () => {
  await using rig = await WireitTestRig.setup();
  await rig.write({
    'package.json': {
      scripts: {
...
test('nonsense WIREIT_FAILURES', async () => {
  await using rig = await WireitTestRig.setup();
  await rig.write({
    'package.json': {
      scripts: {
...
test('github caching without ACTIONS_RESULTS_URL', async () => {
  await using rig = await WireitTestRig.setup();
...
test('github caching but ACTIONS_RESULTS_URL does not end in slash', async () => {
  await using rig = await WireitTestRig.setup();
  const cmd = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('github caching without ACTIONS_RUNTIME_TOKEN', async () => {
  await using rig = await WireitTestRig.setup();
  const cmd = await rig.newCommand();
  await rig.write({
    'package.json': {
