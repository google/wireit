...
test('fresh script is skipped', async () => {
...
test('changing input file makes script stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('directory matched by files array covers recursive contents', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('content of symlink targets affects key', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('freshness check supports glob re-inclusion', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('changing input file modtime does not make script stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('script with undefined input files is always stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('script with undefined output files is always stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('script with undefined input/output files and undefined command can be fresh', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
...
test('script with empty input files can be fresh', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('empty directory is not included in fingerprint', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('cross-package freshness is tracked', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
...
test('input file can be outside of package', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'foo/package.json': {
...
test('two commands which reference the same input file', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdB = await rig.newCommand();
  const cmdC = await rig.newCommand();
  await rig.write({
...
test('glob recursive stars (**) match input files', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('glob negations (!) exclude input files', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('fresh script is skipped with unsafe characters in script name', async () => {
  await using rig = await WireitTestRig.setup();
...
test('failure makes script stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('fingerprint file is deleted before invoking command', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('script is stale if a dependency was stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  const cmdB = await rig.newCommand();
  await rig.write({
...
test('script is always stale if a dependency has no input files', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  const cmdB = await rig.newCommand();
  await rig.write({
...
test('changing script command makes script stale', async () => {
  await using rig = await WireitTestRig.setup();
...
test('changing output glob patterns makes script stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
...
test('changing clean setting makes script stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
...
test('fingerprint is independent of file ordering', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('fingerprint is independent of dependency ordering', async () => {
  await using rig = await WireitTestRig.setup();
  //         a
  //         |
  //    +-+--+--+-+
...
for (const [agent, lockfile] of [
  ...
  test(`changing ${lockfile} with ${agent} changes fingerprint`, async () => {
    await using rig = await WireitTestRig.setup();
    const cmdA = await rig.newCommand();
    await rig.write({
      'foo/package.json': {
...
test('changing package-lock.json does not invalidate when packageLocks is empty', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('changing custom.lock invalidates when set in packageLocks', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('packageLocks can have multiple files', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('leading slash on files glob is package relative', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
...
test('file-only rule affects fingerprint of consumers', async () => {
  await using rig = await WireitTestRig.setup();
...
test('script is not fresh if output file is modified externally', async () => {
  await using rig = await WireitTestRig.setup();
...
test('changing external environment variable makes script stale', async () => {
  await using rig = await WireitTestRig.setup();
  const cmdA = await rig.newCommand();
  await rig.write({
    'package.json': {
